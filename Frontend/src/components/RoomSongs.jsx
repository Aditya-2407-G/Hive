...
export default function Component() {
    ...
    const fetchRoomData = async () => {
        try {
            ...
            setSongs(songsResponse.data.sort((a, b) => b.upvotes - a.upvotes));
        } catch (error) {
    ...
    useEffect(() => {
        ...
        stompClient.onConnect = () => {
            ...
            stompClient.subscribe(`/topic/room/${roomId}/songs`, (message) => {
                ...
                console.log("Received song update:", updatedSongs);
                setSongs((prevSongs) => {
                    const newSong = updatedSongs.find(
                        (song) => !prevSongs.some((s) => s.id === song.id)
                    );
                    if (newSong) {
                        return [...prevSongs, newSong].sort(
                            (a, b) => b.upvotes - a.upvotes
                        );
                    }
                    return updatedSongs.sort((a, b) => b.upvotes - a.upvotes);
                });
            });
            ...
            stompClient.subscribe(
                ...
                (message) => {
                    const { endedSongId, newSongOrder } = JSON.parse(
                        message.body
                    );
                    setSongs((prevSongs) => {
                        const endedSong = prevSongs.find(
                            (song) => song.id === endedSongId
                        );
                        if (endedSong) {
                            const updatedSongs = prevSongs.filter(
                                (song) => song.id !== endedSongId
                            );
                            endedSong.upvotes = 0; // Reset votes
                            updatedSongs.push(endedSong); // Add to the end
                            return newSongOrder.map((id) =>
                                updatedSongs.find((song) => song.id === id)
                            );
                        }
                        return prevSongs;
                    });
        ...
        try {
            ...
            toast({
                ...
                description:
                    ...
                    "Failed to vote on song. Please try again.",
                variant: "destructive",
            });
        } finally {
    ...
    const onSongEnd = async () => {
        if (songs.length > 0) {
            const endedSong = songs[0];
            try {
                await api.post(`/rooms/${roomId}/songs/${endedSong.id}/ended`);
            } catch (error) {
                console.error(
                    "Error notifying server about ended song:",
                    error
                );
            }
        }
                                        ...
                                        <div className="flex justify-between items-center py-2">
                                            ...
                                            <div className="flex items-center space-x-2">
                                                <Button
                                                    ...
                                                    disabled={loadingSongIds.includes(
                                                        song.id
                                                    )}
                                                    variant="ghost"
                                                    size="sm"
